...
class BaseAlert(object):
    ...
    def signal_handler(self, **kwargs):
        ...
        def mk_alert(user, backend, instance=None):
            ...
            data = self.data.update(self.get_data(**kwargs))
            return Alert(
                          user=user,
                          backend=backend.id,
                          alert_type=self.id,
        ...
        alerts = (mk_alert(user, backend, instance=instance) for (user, backend) in AlertPreference.objects.get_recipients_for_notice(self.id, users))
